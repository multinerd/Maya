...
namespace Maya.AspNetCore.TagHelpers.Metronic.Controls.Components
{
    ...
    public class AlertTagHelper : MeconsTagHelperBase
    {
        ...
        protected override async Task RenderProcessAsync(TagHelperContext context, TagHelperOutput output)
        {
            ...
            if (this.IsOutline)
                output.AddCssClass("m-alert--outline");
            ...
            if (!this.DisableLinkStyling)
                output.Content.SetHtmlContent(Regex.Replace((await output.GetChildContentAsync(true)).GetContent(), "<a( [^>]+)?>", new MatchEvaluator(this.AddLinkStyle)));
        }
